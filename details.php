...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $valid_targets = ['fat-loss', 'body-recomposition', 'muscle-gain'];
    $valid_activities = ['bmr', 'sedentary', 'light-exercise', 'moderate-exercise', 'active-exercise', 'very-active-exercise'];

    if (
        $age === false || empty($gender) || !in_array($gender, $valid_genders) ||
        $height === false || $weight === false ||
        empty($target) || !in_array($target, $valid_targets) ||
        empty($activity) || !in_array($activity, $valid_activities) ||
        $target_weight === false
    ) {
        $error = "All fields are required and must contain valid selections.";
    } else {
        ...
        if ($error === null) {
            ...
            $calculated_calorie_goal = round($tdee);

            if ($target === 'fat-loss') {
                $calculated_calorie_goal = round($tdee - 400);
            } elseif ($target === 'muscle-gain') {
                $calculated_calorie_goal = round($tdee + 300);
            }

            $calculated_calorie_goal = max(1200, $calculated_calorie_goal);


            $conn->begin_transaction();
            try {
                $stmt_user = $conn->prepare("UPDATE userstable SET user_age = ?, user_gender = ?, user_height = ?, user_weight = ?, target_weight = ?, calorie = ? WHERE user_email = ?");
                if (!$stmt_user) throw new Exception("Prepare failed (userstable): " . $conn->error);
                $stmt_user->bind_param("issddis", $age, $gender, $height, $weight, $target_weight, $calculated_calorie_goal, $user_email);
...
<body>
    ...
    <div class="form-container">
        ...
        <div class="error-message" <?php if (!$error) echo 'style="display:none;"'; ?>>
        ...
        <form method="POST" action="" id="detailsForm">
            ...
            <div class="button-row">
                <a href="dashboard.php" class="form-button">Back to Dashboard</a>
